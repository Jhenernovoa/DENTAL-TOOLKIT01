...
public class Agregar_cita extends javax.swing.JPanel {
    ...
    public Calendario CITAS = new Calendario();
    public SMS service = new SMS();
    ...
    public void Recuperar() throws SQLException{
        Gson gson = new Gson();
        String br=CITAS.Recuperar();
        if(!br.equals("")){
            Calendario B = gson.fromJson(br, Calendario.class);
            CITAS=B;
            Reconstruir(B);
        }
    }
    
    public void Reconstruir(Calendario B){
        for(int i = 0;i<B.size();i++){
            String Nombre = B.Citas.get(i).Nombre;
            String Razon = B.Citas.get(i).Razon;
            String HoraI = B.Citas.get(i).HoraI;
            String HoraF = B.Citas.get(i).HoraF;
            String Dia = B.Citas.get(i).Dia;
            String caso[] = {Nombre,Razon,HoraI,HoraF,Dia};
            modelo.addRow(caso);
        }
        modelo= (DefaultTableModel) jTable1.getModel();
    }
    ...
    public void GUARDA(){
        try {
                    Gson gson = new Gson();
                    String jsonInString = gson.toJson(CITAS);
                    Calendario OD = new Calendario();
                    if(OD.Guardar(jsonInString)==1){
                        JOptionPane.showMessageDialog(null,"Agenta Actualizada");
                    }else{
    ...
    public void eliminarElemento2(String Correo){
        String Nombre;
        for(int i =0;i<ARR.size();i++){
            if(ARR.get(i).Correo.equals(Correo)){
                Nombre=ARR.get(i).Nombre+" "+ARR.get(i).Apellidos;
                for(int o =0;o<modelo.getRowCount();o++){
                    if(modelo.getValueAt(o,0).equals(Nombre)){
                        datos.clear();
                        datos.add(0, "\nNombre del paciente: " + (String)modelo.getValueAt(o, 0));
                        datos.add(1, "\nRazon de la cita: " + (String)modelo.getValueAt(o, 1));
                        datos.add(2, "\nHora de inicio: " + (String)modelo.getValueAt(o, 2));
                        datos.add(3, "\nHora estimada de fin: " + (String)modelo.getValueAt(o, 3));
                        datos.add(4, "\nDia" + (String)modelo.getValueAt(o, 4));
                        if(JOptionPane.showConfirmDialog(this, "Confirma la cita: " + datos, "CONFIRMAR", JOptionPane.YES_NO_OPTION) == 0){
                            CITAS.Citas.remove(o);
                            modelo.removeRow(o);
                        }
                    }
                }
    ...
    private void initComponents() {
        ...
        jPanel1.add(jButton1, new org.netbeans.lib.awtextra.AbsoluteConstraints(660, 630, 110, 50));
    ...
    private void jButton3MouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_jButton3MouseClicked
        ...
        String [] datos={"","","","",""};
         ...
         datos[1]=jTextArea1.getText();
         datos[2]=jTextField4.getText() + ":" +jTextField5.getText();
         datos[3]=jTextField3.getText() + ":" +jTextField2.getText();
         datos[4]=dateFormat.format(jCalendar1.getDate());
         if((jTextField4.getText().equals( jTextField3.getText()) && jTextField5.getText().equals(jTextField2.getText())) || (Integer.parseInt(jTextField4.getText()) > Integer.parseInt(jTextField3.getText())) || ( jTextField4.getText().equals( jTextField3.getText())&& Integer.parseInt(jTextField5.getText()) > Integer.parseInt(jTextField2.getText())) || Integer.parseInt(jTextField4.getText()) > 24 || Integer.parseInt(jTextField3.getText()) > 24 || Integer.parseInt(jTextField5.getText()) > 60 || Integer.parseInt(jTextField2.getText()) > 60){
             ...
             for(int i = 0; i < jTable1.getRowCount(); i++){ 
                 if(modelo.getValueAt(i, 4).equals(datos[4]) && datos[2].equals( modelo.getValueAt(i, 2))){
                     JOptionPane.showMessageDialog(this, "HORA OCUPADA");
                     return;
                 }
             }
             this.llenarTabla(datos[0], datos[1], datos[2], datos[3], datos[4]);
             CITAS.addCita(new Cita().CrearCita(datos[0], datos[1], datos[2], datos[3], datos[4]));
             //JOptionPane.showMessageDialog(null, "Cita agregada");
                try {
                    ...
                    Calendario OD = new Calendario();
                    if(OD.Guardar(jsonInString)==1){
                        JOptionPane.showMessageDialog(null,"Agenda guardada");
                        for(int i=0;i<ARR.size();i++){
                            System.out.println(datos[0]);
                            System.out.println(ARR.get(i).Nombre);
                            if(datos[0].equals(ARR.get(i).Nombre+" "+ARR.get(i).Apellidos)){
                                service.mandar(ARR.get(i).Celular,ARR.get(i).Nombre+" tu cita fue agendada para el "+datos[4]+" "
                                        + "a las "+datos[2]);
                            }
                        }
                    }else{
    ...
    private void jButton1ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton1ActionPerformed
        ...
        for(int i =0;i<ARR.size();i++){
            if(modelo.getValueAt(row,0).equals(ARR.get(i).Nombre+" "+ARR.get(i).Apellidos)){
                try {
                    service.mandar(ARR.get(i).Celular,"Tu cita del "+modelo.getValueAt(row,4)+" a las "+modelo.getValueAt(row,2)+" ha sido cancelada.");
                } catch (IOException ex) {
                    Logger.getLogger(Agregar_cita.class.getName()).log(Level.SEVERE, null, ex);
                }
            }
